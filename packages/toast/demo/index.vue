 ...
 * @LastEditTime: 2019-08-26 19:53:38
 * @LastEditors: Please set LastEditors
 -->
<template>
  <demo-section>
    <demo-block :title="$t('title1')">
      <eva-button @click="$toast($t('text'))">{{ $t('title1') }}</eva-button>
      <eva-button @click="$toast($t('longText'))">{{ $t('longTextButton') }}</eva-button>
    </demo-block>

    <demo-block :title="$t('title2')">
      <eva-button @click="showLoadingToast">{{ $t('title2') }}</eva-button>
    </demo-block>

    <demo-block :title="$t('title3')">
      <eva-button @click="showSuccessToast">{{ $t('success') }}</eva-button>
      <eva-button @click="showFailToast">{{ $t('fail') }}</eva-button>
    </demo-block>

    <demo-block :title="$t('advancedUsage')">
      <eva-button @click="showCustomizedToast">{{ $t('advancedUsage') }}</eva-button>
    </demo-block>
  </demo-section>
</template>
