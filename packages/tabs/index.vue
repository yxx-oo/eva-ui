 ...
 * @LastEditTime: 2019-10-18 14:53:42
...
<template>
  <div :class="b([type])">
    <div
      ...
      :class="[
        b('wrap', { scrollable }),
        { 'eva-hairline--top-bottom': type === 'line' }
      ]"
    >
      <div
        ref="nav"
      ...
      >
        ...
        <div
          ...
          class="eva-tab"
          :class="{
            'eva-tab--active': index === curActive,
            'eva-tab--disabled': tab.disabled,
            'eva-tab--complete': !ellipsis
          }"
...
export default create({
  ...
  /*
    调用方传入v-model属性值，上面的model的prop:'active'会将active映射到props的active做绑定，
    model这个函数里面默认会包含一个input属性，通过this.$emit('input', value);改变active的值，
    调用方只要在watch中观察值得变化，就可以自动触发调用。具体可查看vue官网的v-model双向绑定属性
  */
  props: {
    color: String,
    sticky: Boolean,
  ...
  methods: {
    ...
    onClick(index) {
      ...
      const computedName = this.tabs[index].index;
      if (disabled) {
